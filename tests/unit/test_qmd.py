...
def test_qmd_handling_stdout_bounds(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp:
        base = Path(tmp)
        manifest_path = base / "current" / "manifest.json"
        manifest_path.parent.mkdir(parents=True)
        ...
        mock_qmd = base / "mock_qmd.sh"
        # Output slightly more than 5MB
        mock_qmd.write_text("#!/usr/bin/env python3\nimport sys\nif 'collection list' in ' '.join(sys.argv):\n  print('hermes-skills')\nelse:\n  sys.stdout.write('x' * 6000000)\n  sys.stdout.flush()")
        mock_qmd.chmod(0o755)
        ...
        assert result["success"] is False
        assert result["code"] == "qmd_error"
        assert "exceeded safe limit" in result["message"]
